...
-- values of type 'b' kept in the leaf nodes, FS attributes of
-- type 'a', FS values of type 'v' and FS variables of type 'r'.
data Tree n l a v r
    = INode -- ^ Interior node
        { labelI    :: n
        , topFS     :: FS a v r
        -- TODO: empty for substitution nodes?
        , botFS     :: FS a v r
        , subTrees  :: [Tree n l a v r] }
    | FNode -- ^ Foot node
        { labelI    :: n
        , topFS     :: FS a v r
        , botFS     :: FS a v r }
...
dummify :: Tree n l a v r -> Tree n l (Either a Int) v r
dummify 
