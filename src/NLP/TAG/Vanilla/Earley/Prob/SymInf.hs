...
-- | Parsed trees generation.  On top of that, symmetric inference rules.
-- One step towards probabilistic parsing.


module NLP.TAG.Vanilla.Earley.Prob.SymInf where
...
tryAdjoinInit' q = void $ P.runListT $ do
    ...
    p <- provideBeg' footNT (q ^. spanA ^. end)
    let pLab = p ^. label
        pSpan = p ^. spanP
    ...
    lift . lift $ do
        ...
        putStr "  :  " >> printActive q'
    -- push the resulting state into the waiting queue
    lift $ pushInduced q' $ Foot q footNT
