...
module NLP.TAG.Vanilla.Earley.Prob.AStar where
...
instance (Eq n, Eq t) => Eq (ExtCost n t) where
    (==) = (==) `on` (addCost <$> prioVal <*> estCost)
instance (Ord n, Ord t) => Ord (ExtCost n t) where
    compare = compare `on` (addCost <$> prioVal <*> estCost)
...
extCost0 :: Cost -> Cost -> ExtCost n t
extCost0 p initEst = ExtCost p initEst S.empty


-- | Construct an `ExtCost` with one incoming traversal
-- (traversal=hyper-edge).
extCost :: Cost -> Cost -> Trav n t -> ExtCost n t
extCost p est = ExtCost p est . S.singleton
...
joinExtCost x y = if estCost x /= estCost y
    then error "joinExtCost: estimation costs differ!"
    else ExtCost
...
mkEarSt
    :: (Ord n, Ord t)
    => Cost         -- ^ Initial estimation for (0, length sent) span
    -> M.Map (Active n t) Cost
    -> EarSt n t
mkEarSt initEst m = EarSt
    { doneActive  = M.empty
    , donePassive = M.empty
    , waiting     = Q.fromList
        [ ItemA p :-> extCost0 c initEst
        | (p, c) <- M.toList m ] }


-- | The Reader component.
data EarEnv t = EarEnv
    { inputSent :: [t]
    -- ^ The input sentence
    , minCosts  :: [Cost]
    -- ^ Minimal costs of parsing individual terminals
    } deriving (Show)
...
type Earley n t = RWS.RWST (EarEnv t) () (EarSt n t) IO
...
-- | Estimate the remaining distnance.
estimateDist :: Span -> Earley n t Cost
estimateDist span = do
    n  <- sentLen
    x <- estOn 0 (span ^. beg)
    y <- estOn (span ^. end) n
    z <- case span ^. gap of
        Nothing -> return costZero
        Just (i, j) -> estOn i j
    return $ x + y + z
  where
    sentLen = length <$> RWS.asks inputSent
    estOn i j = sumCost . over i j <$> RWS.asks minCosts
...
tryScan p cost = void $ runMaybeT $ do
    ...
    lift . pushInduced q . extCost (addCost cost termCost) estDist $
        Scan p (t, termCost)
...
trySubst p cost = void $ P.runListT $ do
    ...
    estDist <- lift . estimateDist $ q' ^. spanA
    -- push the resulting state into the waiting queue
    lift . pushInduced q' . extCost (addCost cost cost') estDist $ Subst p q
...
tryAdjoinInit p _cost = void $ P.runListT $ do
    ...
    estDist <- lift . estimateDist $ q' ^. spanA
    -- push the resulting state into the waiting queue
    lift . pushInduced q' . extCost cost' estDist $ Foot q (nonTerm foot)
...
tryAdjoinInit' q cost = void $ P.runListT $ do
    ...
    estDist <- lift . estimateDist $ q' ^. spanA
    -- push the resulting state into the waiting queue
    lift . pushInduced q' . extCost cost estDist $ Foot q footNT
...
tryAdjoinTerm q cost = void $ P.runListT $ do
    ...
    lift . lift $ do
        ...
        putStr "  :  " >> printPassive p'
    -- compute the estimated distance for the resulting state
    estDist <- lift . estimateDist $ p' ^. spanP
    -- push the resulting state into the waiting queue
    lift . pushPassive p' . extCost (addCost cost cost') estDist $ Adjoin q p
...
_earley
    ...
    -> IO (EarSt n t)
_earley gram xs =
    fst <$> RWS.execRWST loop env0 st0
  where
    -- initial environment
    env0 = EarEnv
        { inputSent = xs
    ...
    st0 = mkEarSt estInit $ M.fromList -- $ Reid.runReid $ mapM reidState
        [ ( Active
            { _root  = headR
            , _right = bodyR
            , _spanA = Span
    ...
    loop = popItem >>= \mp -> case mp of
        Nothing -> return ()
        Just p -> step p >> loop
    -- estInit = estimateCost gram 0 (length xs) xs
    estInit = sumCost estCosts
    estCosts = map (estimateTerm gram) xs
...
-- -- | Estimate remaining distance for the given span based on the given
-- -- sentence.
-- estimateCost
--     :: (Ord n, Ord t)
--     => S.Set (Rule n t)
--     -> Pos -> Pos
--     -> [t] -> Cost
-- estimateCost gram i j
--     = sumCost
--     . map (estimateTerm gram)
--     . over i j


-- | Estimate the minimal cost of scanning the given terminal.
estimateTerm :: (Ord n, Ord t) => S.Set (Rule n t) -> t -> Cost
estimateTerm gram x = minimum
    [ termCost
    | rule <- S.toList gram
    , Term{..} <- bodyR rule
    , term == x ]
