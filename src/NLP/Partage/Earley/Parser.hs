...
pushActive :: (Ord t, Ord n, Ord v) => Active -> Trav n t v -> Earley n t v ()
...
-- is not already in the set of processed (`done') states.
pushPassive :: (Ord t, Ord n, Ord v) => NonActive n v -> Trav n t v -> Earley n t v ()
...
-- | See `Chart.expectEnd`.
expectEnd :: DID -> Pos -> P.ListT (Earley n t v) Active
...
rootSpan
    :: (Ord n) => n -> (Pos, Pos)
    -> P.ListT (Earley n t v) (NonActive n v)
...
tryScan :: (SOrd t, SOrd n, SOrd v) => Active -> Earley n t v ()
...
tryAdjoinInit :: (SOrd n, SOrd t, SOrd v) => NonActive n v -> Earley n t v ()
...
tryAdjoinCont :: (SOrd n, SOrd t, SOrd v) => Passive -> Earley n t v ()
...
tryAdjoinTerm :: (SOrd t, SOrd n, SOrd v) => Top n v -> Earley n t v ()
...
--            $ p
  ...
  where
    ...
    mkPass qSpan (Right p) = do
      -- make sure that the corresponding rule is regular
      guard $ regular (p ^. spanT)
      return . Right $ updateSpan qSpan spanT p

    mkPass qSpan (Left p) = do
      return . Left $ updateSpan qSpan spanP p

    updateSpan qSpan getSpan
      = setL (getSpan >>> beg) (qSpan ^. beg)
      . setL (getSpan >>> end) (qSpan ^. end)
