...
-- | A version in which a separate automaton is built (according to the
-- underlying building function) for each distinct rule head symbol, i.e. the
-- set of rules is first partitioned w.r.t. the heads of the individual rules,
-- and then for each partition a separate automaton is built.


module NLP.Partage.Auto.WeiSet
( fromGram
) where
...
shell :: (Ord a) => AutoSet a -> A.WeiAuto a
shell AutoSet{..} = A.WeiAuto
    { rootsWei = S.fromList
               . map unExID
               . S.toList $ rootSet
    ...
    , followWei = \e x -> do
        ...
        (w, j)      <- A.followWei auto i x
        (w,) . unExID <$>
            M.lookup (autoID, j) toExID
    , edgesWei = \e -> do
        let mtl = maybeToList
        (autoID, i) <- mtl $ M.lookup (ExID e) fromExID
        auto        <- mtl $ M.lookup autoID autoMap
        (x, w, j)   <- A.edgesWei auto i
...
fromGram
    :: (M.Map Rule Weight -> A.WeiGramAuto n t)
...
-- | An external identifier (in contrast to internal identifiers which are local
-- to individual component automata).
...
data AutoSet a = AutoSet
    { autoMap   :: M.Map AutoID (A.WeiAuto a)
...
buildAutoSet mkOne gram = runM $
  ...
  where
    ...
    gramByHead = M.fromListWith M.union
        [ (headR r, M.singleton r rw)
        | (r, rw) <- M.toList gram ]
    -- build a single automatom
    mkAutoSet autoID auto = do
        rootMap <- mkNodeMap (A.rootsWei auto)
        descMap <- mkNodeMap (A.allIDs (A.fromWei auto) S.\\ A.rootsWei auto)
